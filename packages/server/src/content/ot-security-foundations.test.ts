/**
 * Golden tests for OT Security Foundations.
 *
...
const EXERCISES = OT_SECURITY_FOUNDATIONS.modules.flatMap((module) => module.exercises);
...
describe('OT Security Foundations is registered and structured as claimed', () => {
  it('ships in the catalogue', () => {
    expect(PACKAGES.map((pkg) => pkg.id)).toContain('ot-security-foundations');
  });

  it('has 40 exercises across 8 modules', () => {
    expect(OT_SECURITY_FOUNDATIONS.modules).toHaveLength(8);
    expect(EXERCISES).toHaveLength(40);
  });

  it('numbers each module\'s exercises from one, in order', () => {
    for (const module of OT_SECURITY_FOUNDATIONS.modules) {
      ...
      for (const exercise of module.exercises) {
        ...
        expect(exercise.packageId).toBe('ot-security-foundations');
      }
    }
  });

  it('needs no terminal, because there is no simulated PLC and inventing one would teach a fiction', () => {
