/**
 * Golden tests for Identity Foundations.
 *
...
const EXERCISES = IDENTITY_FOUNDATIONS.modules.flatMap((module) => module.exercises);
...
describe('Identity Foundations is registered and structured as claimed', () => {
  it('ships in the catalogue', () => {
    expect(PACKAGES.map((pkg) => pkg.id)).toContain('identity-foundations');
  });

  it('has 40 exercises across 8 modules', () => {
    expect(IDENTITY_FOUNDATIONS.modules).toHaveLength(8);
    expect(EXERCISES).toHaveLength(40);
  });

  it('numbers each module\'s exercises from one, in order', () => {
    for (const module of IDENTITY_FOUNDATIONS.modules) {
      ...
      for (const exercise of module.exercises) {
        ...
        expect(exercise.packageId).toBe('identity-foundations');
      }
    }
  });

  it('needs no terminal, because there is no simulated directory and inventing one would teach a product', () => {
