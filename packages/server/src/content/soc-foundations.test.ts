/**
 * Golden tests for SOC Foundations.
 *
...
const EXERCISES = SOC_FOUNDATIONS.modules.flatMap((module) => module.exercises);
...
describe('SOC Foundations is registered and structured as claimed', () => {
  it('ships in the catalogue', () => {
    expect(PACKAGES.map((pkg) => pkg.id)).toContain('soc-foundations');
  });

  it('has 40 exercises across 8 modules', () => {
    expect(SOC_FOUNDATIONS.modules).toHaveLength(8);
    expect(EXERCISES).toHaveLength(40);
  });

  it('numbers each module\'s exercises from one, in order', () => {
    for (const module of SOC_FOUNDATIONS.modules) {
      ...
      for (const exercise of module.exercises) {
        ...
        expect(exercise.packageId).toBe('soc-foundations');
      }
    }
  });

  it('needs no terminal and no lab, which is what makes it an orientation package', () => {
